  ...
  connectedCallback () {
    ...
    customElements.whenDefined("light-preview").then(() => {
      setTimeout(async () => {
        ...
        if (this.isLocal) {
          const vendored_files = await (await fetch("/content-editable/vendor.json")).json()

          Object.entries(vendored_files).forEach(([filename, content]) => {
            this.files[`vendor/content-editable/${filename}`] = content
          })
        } else {
...
*.sw?`
  ...
  packageJSON () {
    const version = this.isLocal ? "file:./vendor/content-editable" : document.querySelector("meta[name='version-number']").content
    return `{
      "name": "content-editable-starter",
      "private": true,
      "version": "0.0.0",
      "type": "module",
      "scripts": {
